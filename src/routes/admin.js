...
module.exports = async function adminRoutes(fastify) {
  ...
  fastify.get('/admin/stats', async (req, reply) => {
    ...
    `).all();
    return reply.view('stats.njk', { title: 'Stats', layout: 'layouts/admin.njk', allRecords, showUser: true, dataUrl: '/admin/stats/data', hitsUrl: '/admin/stats/hits' });
  });
  ...
  // GET /admin/stats/hits — paginated DNS hit log for selected records (admin, no ownership check)
  fastify.get('/admin/stats/hits', async (req, reply) => {
    const db = getDb();
    ...
    `).all(...ids, fromDate, toDate, perPage, offset);
